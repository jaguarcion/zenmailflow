...
export function insertJetBrainsAccount(email, password, license_email) {
  const expiresAt = new Date();
  expiresAt.setFullYear(expiresAt.getFullYear() + 1);
  const stmt = db.prepare('INSERT INTO jetbrains_accounts (email, password, license_email, expires_at) VALUES (?, ?, ?, ?)');
  return stmt.run(email, password, license_email, expiresAt.toISOString());
}

export function getAllJetBrainsAccounts() {
  const stmt = db.prepare('SELECT * FROM jetbrains_accounts ORDER BY created_at DESC');
  return stmt.all();
}

export function insertJetBrainsStudentEmail(email, password) {
  try {
    const stmt = db.prepare('INSERT INTO jetbrains_student_emails (email, password, status) VALUES (?, ?, ?)');
    return stmt.run(email, password, 'pending');
  } catch (err) {
