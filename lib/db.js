...
export function deleteJetBrainsOrder(id) {
  const tx = db.transaction(() => {
    db.prepare('UPDATE jetbrains_student_emails SET order_id = NULL WHERE order_id = ?').run(id);
    db.prepare('UPDATE jetbrains_accounts SET order_id = NULL WHERE order_id = ?').run(id);
    db.prepare('DELETE FROM jetbrains_orders WHERE id = ?').run(id);
  });
  tx();
}
...
export function getDashboardStats() {
  ...
  // ESET Stats
  const esetStats = db.prepare(`SELECT COUNT(*) as total_emails, SUM(CASE WHEN used = 1 THEN 1 ELSE 0 END) as used_emails FROM eset_emails`).get();
  const esetTgUsers = db.prepare(`SELECT COUNT(*) as count FROM eset_tg_users`).get().count;

  // JetBrains Stats
  const jetbrainsStats = db.prepare(`SELECT COUNT(*) as total_accounts FROM jetbrains_accounts`).get();
  const jetbrainsOrders = db.prepare(`SELECT COUNT(*) as total_orders, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed_orders FROM jetbrains_orders`).get();

  // Apple Stats
  let appleStats = { total_accounts: 0 };
  try {
    appleStats = db.prepare(`SELECT COUNT(*) as total_accounts FROM apple_accounts`).get();
  } catch (e) {
    // Graceful fallback if table is somehow missing
  }

  // Randmail Stats
  let burpStats = { total_addresses: 0 };
  try {
    burpStats = db.prepare(`SELECT COUNT(*) as total_addresses FROM burp_addresses`).get();
  } catch (e) {
  }

  return {
    accounts: accountsStats, // Adobe stats (legacy name for backward compatibility)
    totalClients,
    clientsGrowth,
    eset: {
      totalKeys: esetStats.total_emails || 0,
      usedKeys: esetStats.used_emails || 0,
      tgUsers: esetTgUsers || 0
    },
    jetbrains: {
      totalAccounts: jetbrainsStats.total_accounts || 0,
      totalOrders: jetbrainsOrders.total_orders || 0,
      completedOrders: jetbrainsOrders.completed_orders || 0
    },
    apple: {
      totalAccounts: appleStats.total_accounts || 0
    },
    burp: {
      totalAddresses: burpStats.total_addresses || 0
    }
  };
}
...
// Apple Accounts Helpers
export function insertAppleAccount(email, emailLogin, password, firstName, lastName, birthDate, phone) {
  const stmt = db.prepare('INSERT INTO apple_accounts (email, email_login, password, first_name, last_name, birth_date, phone) VALUES (?, ?, ?, ?, ?, ?, ?)');
  return stmt.run(email, emailLogin, password, firstName, lastName, birthDate, phone).lastInsertRowid;
}

export function getAllAppleAccounts() {
  const stmt = db.prepare('SELECT * FROM apple_accounts ORDER BY created_at DESC');
  return stmt.all();
}
