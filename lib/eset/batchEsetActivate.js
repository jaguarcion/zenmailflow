...
const runSingleActivation = async (proxyUrls, taskId, workerLogger) => {
  ...
  for (let attempt = 1; attempt <= MAX_PROXY_RETRIES; attempt++) {
    ...
    try {
      const emailApi = await getEmailProvider(proxyUrl);
      ...
      // Return item to be saved by the caller
      return { success: true, item: licenseData };
    } catch (err) {
      ...
      return { success: false, error: err.message || 'Unknown error' }; // Exhausted retries or non-proxy error
    }
  }
...
export const startBatchEsetActivate = async (taskId, totalCount = 10, concurrency = 2) => {
  ...
  for (let i = 0; i < totalCount; i++) {
    ...
    const p = runSingleActivation(proxyUrls, taskId, workerLogger).then(result => {
      try {
        ...
        if (task) {
          ...
          if (result && result.success) {
            ...
            items.push({ isError: true, error: (result && result.error) || 'Unknown error' });
            db.prepare('UPDATE eset_tasks SET error = error + 1, items_json = ? WHERE id = ?').run(JSON.stringify(items), taskId);
          }
        }
      } catch (e) {
