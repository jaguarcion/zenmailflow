      ...
      if (type === 'fetch' || type === 'xhr' || type === 'document') {
        try {
          ...
          if (text.includes('JetBrains') || text.includes('shop/eform/students/request')) {
            const regexWithG = new RegExp(searchRegex.source, searchRegex.flags.includes('g') ? searchRegex.flags : searchRegex.flags + 'g');
            const matches = [...text.matchAll(regexWithG)];
            if (matches.length > 0) {
    ...
    if (!isMicrosoft) {
      ...
      while (Date.now() - startTime < maxWaitMs && !foundMatch) {
        try {
          ...
          const regexWithG = new RegExp(searchRegex.source, searchRegex.flags.includes('g') ? searchRegex.flags : searchRegex.flags + 'g');
          const matches = [...bodyText.matchAll(regexWithG)];
          if (matches.length > 0) { 
            const lastMatch = matches[matches.length - 1];
            foundMatch = lastMatch[1] || lastMatch[0]; 
            break; 
