...
import { getAuthSessionId, getRecoveryOtp, confirmJetBrainsLicense as confirmLicense, getAcceptLink } from './outlookScraper.js';
        ...
        for (const msg of inboxRes.value) {
          ...
          if (msgRes.isOk() && msgRes.value) {
            ...
            if (msg.from && msg.from.toLowerCase().includes('jetbrains')) {
               ...
               // Usually JetBrains OTP is a 6 digit number
               const otpMatch = msgRes.value.match(/(\d{6})/);
               if (otpMatch) {
                 console.log(`[JB Worker] Extracted OTP: ${otpMatch[1]}`);
                 return otpMatch[1];
               } else {
                 console.log(`[JB Worker] Could not find 6 digits in JetBrains email! Body snippet: ${msgRes.value.substring(0, 100)}`);
               }
            } else {
