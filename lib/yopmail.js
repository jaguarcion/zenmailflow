...
const PROXY_URL = "https://sx-list.org/NK5fxgZA7Wew5f6hh3NcBvBl8kPGhAf7.txt?limit=20";
let PROXIES = [];
let LAST_PROXY_UPDATE = 0;

async function getProxyList() {
    const now = Date.now();
    // Кешируем на 10 минут (600000 мс)
    if (now - LAST_PROXY_UPDATE > 600000 || PROXIES.length === 0) {
        try {
...
async function getYopmailAlias(targetUrl, headless = true, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        // На последней попытке пробуем без прокси как запасной вариант
        const proxyObj = attempt === maxRetries ? null : await getRandomProxy();
        let browser;
