...
export async function startAppleRegistration(jobId, count) {
    ...
    const appendLog = async (args) => {
        const msg = args.map(a => (typeof a === 'object' && a !== null) ? (a instanceof Error ? a.message : JSON.stringify(a)) : a).join(' ');
        const timestamp = new Date().toLocaleTimeString('ru-RU');
        queue.logs.push(`[${timestamp}] ${msg}`);
        if (queue.logs.length > 200) queue.logs.shift(); // Keep last 200 logs max
        await redis.set('apple_queue_status', JSON.stringify(queue));
    };

    console.log = (...args) => {
        originalLog(...args);
        appendLog(args).catch(console.error);
    };
