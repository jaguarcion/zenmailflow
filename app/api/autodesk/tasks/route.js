...
export async function GET(request) {
    ...
    try {
        ...
        const totalCount = db.prepare('SELECT COUNT(*) as c FROM autodesk_tasks').get().c;
...
export async function DELETE(request) {
    ...
    try {
        ...
        if (id === 'all') {
            db.prepare('DELETE FROM autodesk_tasks').run();
        } else {
            db.prepare('DELETE FROM autodesk_tasks WHERE id = ?').run(id);
        }
